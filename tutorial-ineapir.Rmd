...
Por ejemplo, para la tabla [Índices nacionales: general y de grupos ECOICOP](https://www.ine.es/jaxiT3/Tabla.htm?t=76125) (Id 76125):


```{r tabla1, exercise=TRUE, exercise.eval=FALSE}
tabla <- get_data_table(idTable = 76125, tip = "A")
...
tabla <- get_data_table(idTable = 76125, tip = "A", nlast = 5)
head(tabla[,c("Nombre","Data")])
```
...
tabla <- get_data_table(idTable = 76125, tip = "A", nlast = 5, unnest = TRUE)
...
tabla <- get_data_table(idTable = 76125, filter = filter, unnest = TRUE,
...
tabla <- get_data_table(idTable = 76125, filter = filter, unnest = TRUE,
                      tip = "A", validate = FALSE,
                      dateStart = "2025/01/01", dateEnd = "2025/12/01")
tabla[,c("Nombre", "T3_Periodo", "Anyo", "Valor")]
```
...
tabla <- get_data_table(idTable = 76125, filter = filter, unnest = TRUE,
                      tip = "A", validate = FALSE,
                      dateStart = c("2024/01/01", "2025/01/01", "2026/01/01"), 
                      dateEnd = c("2024/01/01", "2025/01/01", "2026/01/01"))
tabla[,c("Nombre", "T3_Periodo", "Anyo", "Valor")]
```
...
tabla <- get_data_table(idTable = 76125, filter = filter, unnest = TRUE,
                      tip = "A", validate = FALSE,
                      dateStart = c("2024/01/01", "2025/01/01", "2026/01/01"),
                      dateEnd = c("2024/03/01", "2025/03/01", "2026/03/01"))
tabla[,c("Nombre", "T3_Periodo", "Anyo", "Valor")]
```
...
tabla <- get_data_table(idTable = 76125, filter = filter, unnest = TRUE,
                      tip = "A", validate = FALSE,
                      dateStart = c("2024/01/01", "2025/01/01", "2026/01/01"), 
                      dateEnd = c("2024/03/01", "2025/03/01"))
tabla[,c("Nombre", "T3_Periodo", "Anyo", "Valor")]
```
...
1. Obtener información de las variables y valores de la tabla de interés, por ejemplo [Índices por comunidades autónomas: general y de grupos ECOICOP](https://www.ine.es/jaxiT3/Tabla.htm?t=76136) (Id = 76136).

```{r tabla5, exercise=TRUE}
metadata <- get_metadata_table_varval(idTable = 76136)
...
ipc <- get_data_table(idTable = 76136, filter = filter, unnest = TRUE,
...
#### Ejemplo 3
Por ejemplo, para la tabla [PIB pm Oferta (Indices de volumen encadenado)](https://www.ine.es/jaxiT3/Tabla.htm?t=67822) (Id 67822). Podemos
...
Por ejemplo, para la serie con código [IPC290750](https://www.ine.es/consul/serie.do?s=IPC290750)
...
series <- get_data_series(codSeries = "IPC290750", tip = "A", nlast = 5, unnest = TRUE)
series
```
...
series <- get_data_series(codSeries = "IPC290750", tip = "A", unnest = TRUE, dateStart = "2023/01/01", 
                          dateEnd = "2023/04/01")
series
```
...
series <- get_data_series(codSeries = "IPC290750", tip = "A", unnest = TRUE, dateStart = "2024/01/01")
series
```
...
1.  Primera parte de la consulta: Grupos o combos de selección que definen a la tabla "[Índices por comunidades autónomas: general y de grupos ECOICOP](https://www.ine.es/jaxiT3/Tabla.htm?t=76136)" (Id 76136)

```{r infotab2, exercise=TRUE}
groups <- get_metadata_table_groups(idTable = 76136)
groups
```    

2.  Segunda parte de la consulta: Valores del grupo "Comunidades y Ciudades Autónomas" (Id=155612) perteneciente a la tabla "[Índices por comunidades autónomas: general y de grupos ECOICOP](https://www.ine.es/jaxiT3/Tabla.htm?t=76136)" (Id=76136)

```{r infotab3, exercise=TRUE}
values <- get_metadata_table_values(idTable = 76136, idGroup = 155612)
values
```
...
values <- get_metadata_table_varval(idTable = 76136)
values
```
...
Por ejemplo, de la serie que recoge los datos del índice general del IPC ([IPC290750](https://www.ine.es/consul/serie.do?s=IPC290750)) que recoge la variación anual del Índice de precios de consumo
...
Por ejemplo, de la serie que recoge los datos del índice general del IPC ([IPC290750](https://www.ine.es/consul/serie.do?s=IPC290750))
...
Por ejemplo, las series de la tabla "[Índices por comunidades autónomas: general y de grupos ECOICOP](https://www.ine.es/jaxiT3/Datos.htm?t=76136)" de la operación IPC (Id 76136).

```{r infoserie3, exercise=TRUE}
series <- get_metadata_series_table(idTable = 76136)
head(series)
```
...
# Obtenemos los datos filtrados 
series <- get_metadata_series_table(idTable = 76136, filter = filter)
series
```
...
## Limitaciones servicio web API JSON
Cuando se realizan consultas de datos de tablas, existen restricciones de volumen
en el número de datos que devuelve la API. Por ejemplo, cuando consultamos todos
los datos de la siguiente tabla aparece el mensaje: 
"No puede mostrarse por restricciones de volumen".

```{r volumen1, exercise = TRUE}
t <- get_data_table(idTable = 56938, nlast = 1)
```
En este caso, tendremos que realizar varias consultas a la tabla utilizando
el filtro de datos. En primer lugar vamos a ver que dimensiones contiene la tabla.

```{r volumen2, exercise = TRUE}
# Obtenemos los id de los tabla grupos (dimensiones)
grupos <- get_metadata_table_groups(idTable = 56938)
grupos
```

Ahora, por ejemplo, vamos a ver los valores del grupo nacionalidad.

```{r volumen3, exercise = TRUE}
# Obtenemos los id de las variables y valores del tablagrupo de nacionalidad
nacionalidad <- get_metadata_table_values(idTable = 56938, idGroup = 113349)
nacionalidad
```

Podemos utilizar la nacionalidad para dividir la consulta inicial y realizar
en su lugar 14 consultas diferentes, una por cada valor de nacionalidad.

```{r volumen4, exercise = TRUE}
# Obtenemos los id de los tabla grupos (cajas)
grupos <- get_metadata_table_groups(idTable = 56938)
...
datos <- NULL
for(i in 1:nrow(nacionalidad[1:2,])){
  filtro <- list()
  filtro[[as.character(nacionalidad$Fk_Variable[i])]] <- nacionalidad$Id[i]
  
  # Consultamos la tabla para el filtro de nacionalidad
  # el unnest de la columna Data lo dejamos para el final porque UE28 no tiene datos
  tmp <- get_data_table(idTable = 56938, nlast = 1, filter = filtro, validate = FALSE)

  # Vamos concatenando las diferentes consultas
  datos <- rbind(datos, tmp)
  
  cat(sprintf("- Consulta %s/%s. Nacionalidad: %s (%s filas | %s filas totales)\n", i, nrow(nacionalidad), nacionalidad$Nombre[i], nrow(tmp), nrow(datos)))
}
```
