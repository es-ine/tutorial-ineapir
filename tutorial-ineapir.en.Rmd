...
To get all the data of a table it is necessary to set the `idTable` argument. For instance, for the table [National indices: overall and by ECOICOP group](https://www.ine.es/jaxiT3/Tabla.htm?t=76125&L=1) (Id 76125):


```{r tabla1, exercise=TRUE, exercise.eval=FALSE}
tabla <- get_data_table(idTable = 76125, tip = "A", lang = "EN")
...
tabla <- get_data_table(idTable = 76125, tip = "A", nlast = 5, lang = "EN")
head(tabla[,c("Nombre","Data")])
```
...
tabla <- get_data_table(idTable = 76125, tip = "A", nlast = 5, unnest = TRUE, lang = "EN")
...
#### Example 1
We are going to filter the table [National indices: overall and by ECOICOP group](https://www.ine.es/jaxiT3/Tabla.htm?t=76125&L=1) (Id 76125):
...
To get the last data of a series it is necessary to set the `codSeries` argument. For example, the time series [IPC290750](https://www.ine.es/consul/serie.do?s=IPC290750)

```{r series1, exercise=TRUE}
series <- get_data_series(codSeries = "IPC290750", tip = "A", lang = "EN")
series$Data
```
...
series <- get_data_series(codSeries = "IPC290750", tip = "A", nlast = 5, unnest = TRUE, lang = "EN")
series
```
...
series <- get_data_series(codSeries = "IPC290750", tip = "A", unnest = TRUE, dateStart = "2024/01/01", lang = "EN")
series
```
...
groups <- get_metadata_table_groups(idTable = 76136, lang = "EN")
groups
```    

2.  The second step is to query the values for an specific group using the function [get_metadata_table_values()](https://es-ine.github.io/ineapir/reference/get_metadata_table_values.html). For instance, values of the group "Comunidades y Ciudades Autónomas" (Id=155612) of the table "[Indices by Autonomous Community: overall and by ECOICOP group](https://www.ine.es/jaxiT3/Tabla.htm?t=76136&L=1)" (Id=76136)
...
To obtain information about a particular time series it is necessary to set the `codSeries` argument, which is the identification code of the series. For example, for the time series of code [IPC290750](https://www.ine.es/consul/serie.do?s=IPC290750).

```{r infoserie1, exercise=TRUE}
series <- get_metadata_series(codSeries = "IPC290750", lang = "EN")
...
values <- get_metadata_series_values(codSeries = "IPC290750", lang = "EN")
values
```
...
series <- get_metadata_series_table(idTable = 76136, lang = "EN")
head(series)
```
...
# Request data using the filter
series <- get_metadata_series_table(idTable = 76136, lang = "EN", filter = filter)
series
```
...
ipc <- get_data_table(idTable = 76125, filter = filter, tip = "A", nlast = 3, lang = "EN")
